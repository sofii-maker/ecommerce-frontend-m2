...
function addToCart(productId, name, price, image, currency, quantity = 1) {
    ...
    if (cartItem) {
        ...
        cart.push({ id: productId, name: name, price: parseFloat(price), quantity: quantity, image: image, currency: currency });
    }
    saveCart();
    updateCartCount();
}
...
function displayDetailPage() {
    ...
    document.getElementById('detail-price-display').textContent = `Precio: ${product.currency} ${product.price}`;
...
function renderCartPage() {
    ...
    cart.forEach(item => {
        ...
        li.innerHTML = `
            <img src="${imageUrl}" alt="${item.name}" style="width: 50px; height: 50px;">
